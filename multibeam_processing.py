...
def create_interpolated_coords(sum_df, gps_gdf):
    ...
    args:
        sum_df typ: beschreibung
        gps_gdf geodataframe: bsechreibung
    returns:
        typ: beschreibung
...
def detect_and_remove_faulty_depth(
    geodf_projected: gpd.GeoDataFrame, window_size: int = 10, threshold: float = 0.5
):  # change according to needs and survey circumstances
    """
    ...
    faulty_gdf = gpd.GeoDataFrame(
        faulty_df,
        geometry=gpd.points_from_xy(faulty_df["Longitude"], faulty_df["Latitude"]),
        crs="EPSG:25833",
    )
...
def detect_and_remove_faulty_depths(
    ...
    # set True / False if autoamtic correction should be applied - if True existing csv with faulty depths will be overwritten
